...
with open('inp/input_04.txt','r') as file:
    inp = file.read()
...
pas = [line.split(' ') for line in passports if contains_all(line,required_fields)==1]
...
import re
def hcl_match(strg, search=re.compile(r'[^a-f0-9#]').search):
    return not bool(search(strg))
def pid_match(strg, search=re.compile(r'[^0-9]').search):
    return not bool(search(strg))
